    ...
    <UCard class="mb-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div v-if="business.facebookUsername" class="flex items-center gap-2">
          ...
          <UButton variant="link" as="a" :to="getPlatformProfileUrl('facebook', business.facebookUsername)" target="_blank">Facebook</UButton>
        </div>
        
        <div v-if="business.instagramUsername" class="flex items-center gap-2">
          <UIcon name="i-mdi-instagram" class="text-pink-600" />
          <UButton variant="link" as="a" :to="getPlatformProfileUrl('instagram', business.instagramUsername)" target="_blank">Instagram</UButton>
        </div>
        
        <div v-if="business.twitterUsername" class="flex items-center gap-2">
          <UIcon name="i-mdi-twitter" class="text-blue-400" />
          <UButton variant="link" as="a" :to="getPlatformProfileUrl('x', business.twitterUsername)" target="_blank">Twitter</UButton>
        </div>
        
        <div v-if="business.youtubeUsername" class="flex items-center gap-2">
          <UIcon name="i-mdi-youtube" class="text-red-600" />
          <UButton variant="link" as="a" :to="getPlatformProfileUrl('youtube', business.youtubeUsername)" target="_blank">YouTube</UButton>
        </div>

        <div v-if="business.tiktokUsername" class="flex items-center gap-2">
          <UIcon name="i-mdi-tiktok" class="text-black" />
          <UButton variant="link" as="a" :to="getPlatformProfileUrl('tiktok', business.tiktokUsername)" target="_blank">TikTok</UButton>
        </div>
      </div>
    </UCard>
